...
const ForgotPassword = ({ toggleForm }) => {
  ...
  const [showModal, setShowModal] = React.useState(false);
  const [showEmailError, setShowEmailError] = React.useState(false);
  const [showVerificationCodeError, setShowVerificationCodeError] =
...
=======
      const response = await axios.post(
        "http://127.0.0.1:8000/api/v1/password/sendOTP",
        {
          email: data.email,
        }
      );
      ...
      console.error("Error:", error);
      setModalMessage(
        error.response?.data?.message || "Có lỗi xảy ra khi kết nối tới server."
      );
>>>>>>> origin/main
    } finally {
      setShowModal(true);
              ...
              <div className="mb-3 input-container">
                <input
                  ...
                  className={`form-control ${
                    errors.verification_code ? "is-invalid" : ""
                  }`}
                  placeholder="Nhập mã xác thực"
