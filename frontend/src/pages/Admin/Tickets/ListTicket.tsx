...
import { StatusType, Tickets } from "../../../interfaces/Ticket";

const ListTicket = () => {
  const { tickets, onDel, onVerify } = useContext(TicketsCT);
  const [StatusList] = useState(Object.values(StatusType));
  const handleVerify = (id: number) => {
    onVerify(id); // Gọi hàm xác minh từ context
  };
  return (
    <div>
                ...
                <tr key={item.id}>
                  ...
                  <td className="p-4 border border-gray-300">
                    <div className="flex items-center space-y-1 justify-between ">
                      ...
                      <button onClick={() => onDel(Number(item.id))} className="mx-1">
                        <svg
                      ...
                      <button onClick={() => handleVerify(Number(item.id))}>
                        <svg
